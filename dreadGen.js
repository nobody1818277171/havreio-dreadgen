...
const wepPath = 'weps/';
const presetPath = "resource/preset.json";

const johnDreadnoughtPath = "resource/Dreadnoughts.json";
...
try {
  ...
  bodies.forEach(element => {
    let body = JSON.parse(fs.readFileSync(bodyPath.concat(element), 'utf8'));
    addMissing(body)
    addMeta(body)
    gotBodies.push(body);
  ...
  weps.forEach(element => {
    let wep = JSON.parse(fs.readFileSync(wepPath.concat(element), 'utf8'));
    addMissing(wep)
    addMeta(wep)
    gotWeps.push(wep);
  ...
  try {
  fs.writeFileSync('out/autogenerated.json', JSON.stringify(presetJson, null, 2));
    } catch (err) {console.log(err.message)}
} catch (err) {
  console.error(err);
}